...
function incidentCreateIntent(assistant) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        incidentData.CreateIncident(postData)
            .then(function (data) {
                var speech = "Great!! Your ticket " + data.number + "was created which describes on " + data.short_description + " under " + data.category + " category with Urgency level " + constants.getDescription(constants.Urgency, data.urgency) + ", last updated on  " + data.sys_updated_on + " and updated by " + data.sys_updated_by;
                resolve(assistant.tell(speech));
            }, function (err) {
